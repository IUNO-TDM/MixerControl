...
export class AdminServiceComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.adminService.getStatisticsEnabled().subscribe(enabled => this.statisticsEnabled = enabled);
    ...
    statisticsEnabledChanged(event) {
        this.adminService.setStatisticsEnabled(event.checked).subscribe();
    }
}
